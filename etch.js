...
function createSquareDiv(numberSqPerSide) {
    ...
    // to check what is the exact number calculated in terms of decimal places.
    let toCalcHeightWidth = 600 / measureHeightWidth;
    //   console.log("the " + measureHeightWidth + " Sq Per Side, the division: " + toCalcHeightWidth);

    // https://www.willmaster.com/library/features/rounding-numbers-with-javascript.php --> This shows how to round number using JavaScript.

    let toRoundNumber = Math.floor(toCalcHeightWidth * 10) / 10; // this fixes the 7x7 and 11x11 Square Grids.
  //  console.log("the " + measureHeightWidth + " Sq Per Side, the division with Round Number: " + toRoundNumber);
    ...
    for (startSquareDiv = 1; startSquareDiv <= endSquareDiv; startSquareDiv++) {
        ...
        for (divCell = 1; divCell <= endSquareDiv; divCell++) {
            ...
            divCellColumn.textContent = divCell; // this is to check if the Square Grids are working. 
            contentSquareDiv.appendChild(divCellColumn);
            divCellColumn.style.height = toRoundNumber + "px";
            divCellColumn.style.width = toRoundNumber + "px";
